...
public class StopperController {
    ...
    public Result<Page<Stopper>> page(@RequestParam(defaultValue = "1") Integer pageNum,
        ...
        if (equipment != null && !equipment.isEmpty()) {
            wrapper.eq(Stopper::getAdaptEquipment, equipment);
        }
        if (status != null) {
    ...
    public Result<Object> add(@RequestBody Map<String, Object> params) {
        ...
        if (!autoGenerate) {
            Stopper existing = stopperService.findExistingByStopperNo(stopper.getStopperNo(), null);
            if (existing != null) {
                Map<String, Object> existingInfo = new HashMap<>();
                existingInfo.put("spec", existing.getSpec());
                existingInfo.put("station", existing.getStation());
    ...
    public Result<Object> update(@RequestBody Stopper stopper) {
        ...
        Stopper existing = stopperService.findExistingByStopperNo(stopper.getStopperNo().trim(), stopper.getId());
        if (existing != null) {
            Map<String, Object> existingInfo = new HashMap<>();
            existingInfo.put("spec", existing.getSpec());
            existingInfo.put("station", existing.getStation());
